        ...
        sortedItems.forEach(item => {
            ...
            if (currentViewMode === 'ACTIVE') {
                if (item.status === 'Pending') {
                    ...
                    actionsHtml += `<button class="btn btn-sm" style="background:var(--border); color:var(--text-main)" onclick="markCustomStatus('${item.id}', '${item.folderId}', 'Archived')">Archive 📦</button>`;
                }
                actionsHtml += `
                    <button class="btn btn-sm btn-outline" onclick="openEditCommitmentModal('${item.id}', '${item.folderId}')">Edit</button>
                    <button class="btn btn-sm btn-danger" onclick="deleteCommitmentEntry('${item.id}', '${item.folderId}')">Delete</button>
                 `;
            } else {
                // Archived View Actions
                actionsHtml += `<button class="btn btn-sm btn-outline" onclick="markCustomStatus('${item.id}', '${item.folderId}', 'Paid')">Unarchive</button>`;
                actionsHtml += `<button class="btn btn-sm btn-danger" onclick="deleteCommitmentEntry('${item.id}', '${item.folderId}')">Delete</button>`;
            }
